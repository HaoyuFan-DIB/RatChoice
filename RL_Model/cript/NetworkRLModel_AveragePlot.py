...
if __name__ == "__main__":
    sub_all = [11, 13, 16, 19, 21, 24] # and 15 to be fit
    #sub_all = [9, 10, 12, 14, 17, 20, 22, 23]
    group_name = "C21"

    # Get exp data first
    Exp_SeqA, Exp_SeqB = np.zeros([len(sub_all), 28]), np.zeros([len(sub_all), 28])
    for i, sub in enumerate(sub_all):
        data = ExpData(RatID=sub)
        Exp_SeqA[i], Exp_SeqB[i] = data.getRLCorr(n_bins=1)

    Exp_SeqA_Avg = np.average(Exp_SeqA, axis=0)
    Exp_SeqA_Std = np.std(Exp_SeqA, axis=0) / np.sqrt(len(sub_all))
    Exp_SeqB_Avg = np.average(Exp_SeqB, axis=0)
    Exp_SeqB_Std = np.std(Exp_SeqB, axis=0) / np.sqrt(len(sub_all))

    # Get model data
    result_csv = "/media/zhemengwu/Gigantic Data/SingleContextSequence/NetworkRLModel_Result.csv"
    Model_SeqA, Model_SeqB = np.zeros([len(sub_all), 28]), np.zeros([len(sub_all), 28])
    counter = 0
    with open(result_csv) as f:
        ...
        while True:
            ...
            if int(tmp_str_split[0]) in sub_all:
                alpha = float(tmp_str_split[1])
                sigma = float(tmp_str_split[2])
                v = float(tmp_str_split[3])
                W_L = [float(tmp_str_split[4]), float(tmp_str_split[5]), float(tmp_str_split[6])]
                W_R = [float(tmp_str_split[7]), float(tmp_str_split[8]), float(tmp_str_split[9])]
                rat = Participant(alpha=alpha, c=0, v=v, sigma=sigma, W_L=W_L, W_R=W_R, use_multi=True)
                rat.evolve(bin_trials=80, n_bins=28)

                L_Corr = rat.Corr_Left[0:-1]
                L_Corr = np.array_split(L_Corr, 28)
                Model_SeqA[counter] = np.array([np.mean(item) for item in L_Corr])

                R_Corr = rat.Corr_Right[0:-1]
                R_Corr = np.array_split(R_Corr, 28)
                Model_SeqB[counter] = np.array([np.mean(item) for item in R_Corr])

                counter = counter + 1

    Model_SeqA_Avg = np.average(Model_SeqA, axis=0)
    Model_SeqA_Std = np.std(Model_SeqA, axis=0) / np.sqrt(len(sub_all))
    Model_SeqB_Avg = np.average(Model_SeqB, axis=0)
    Model_SeqB_Std = np.std(Model_SeqB, axis=0) / np.sqrt(len(sub_all))
    ...
    ax.plot(x, Exp_SeqA_Avg, color="r", marker=".", linestyle="dotted", label="Seq A Exp")
    ax.plot(x, Exp_SeqB_Avg, color="b", marker=".", linestyle="dotted", label="Seq B Exp")
    for i in range(28):
        ax.plot([x[i], x[i]], [Exp_SeqA_Avg[i]+Exp_SeqA_Std[i], Exp_SeqA_Avg[i]-Exp_SeqA_Std[i]], linestyle="--", color="r")
        ax.plot([x[i], x[i]], [Exp_SeqB_Avg[i]+Exp_SeqB_Std[i], Exp_SeqB_Avg[i]-Exp_SeqB_Std[i]], linestyle="--", color="b")
    #ax.grid()
    #ax.legend()
    #ax.set_ylabel("Exp")

    #ax = fig.add_subplot(2,1,2)
    ax.plot(x, Model_SeqA_Avg, color="r", marker="*", label="Seq A Model")
    ax.plot(x, Model_SeqB_Avg, color="b", marker="*", label="Seq B Model")
    for i in range(28):
        ax.plot([x[i], x[i]], [Model_SeqA_Avg[i]+Model_SeqA_Std[i], Model_SeqA_Avg[i]-Model_SeqA_Std[i]], color="r")
        ax.plot([x[i], x[i]], [Model_SeqB_Avg[i]+Model_SeqB_Std[i], Model_SeqB_Avg[i]-Model_SeqB_Std[i]], color="b")
    ax.grid()
    ax.legend()
