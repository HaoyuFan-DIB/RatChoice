...
class Participant():
    def __init__(self,
        ...
        try:
            a = alpha[1]
            self.alpha = np.array(alpha[0:2])
        except:
            self.alpha = np.array([alpha]*2)
        # self.alpha: 2 element adday, for alpha_left and alpha_right
    ...
    def __getAlphas(self):
        if self.use_multi:
            alpha_L = self.alpha[0] * self.W_L
            alpha_R = self.alpha[1] * self.W_R
        else:
            alpha_L = self.alpha[0] * np.ones(3)
            alpha_R = self.alpha[1] * np.ones(3)
...
class Fitter():
    ...
    def __autoParameter(self):
        self.alpha1_array = np.arange(start=0.001, stop=0.012, step=0.002)
        self.alpha2_array = None
        ...
        self.W_port_R = np.arange(start=0.01, stop=5.1, step=0.5)
        self.W_other = np.arange(start=0.01, stop=0.51, step=0.1)
    ...
    def __initAlpha(self):
        while True:
            respond = input("Use different Alphas for S+ and S- trails?[Y/N]")
            if respond.upper() in ["Y", "N"]:
                break
        if respond.upper() == "Y":
            self.alpha1_array = self.__initParameter("Alpha1")
            self.alpha2_array = self.__initParameter("Alpha2")
        else:
            self.alpha1_array = self.__initParameter("Alpha")
            self.alpha2_array = [None]
    ...
    def fit(self):
        best_DF = pd.DataFrame(columns=["Alpha1", "Alpha2", "Sigma", "v", "dW", "dW_portion", "W_Port_R", "W_other", "SSE"])
        starttime = time.time()
        batch_finished = 0
        batch_tot = len(self.alpha1_array)*len(self.alpha2_array)*len(self.sigma_array)*len(self.v_array)
        for alpha1 in self.alpha1_array:
            for alpha2 in self.alpha2_array:
                if alpha2 is None:
                    alpha2 = alpha1
                for sigma in self.sigma_array:
                    for v in self.v_array:
                        print("Fitting {m} / {n} batches...".format(m=batch_finished+1, n=batch_tot))
                        tmp_DF = self.__fitSingleBatch(alpha1, alpha2, sigma, v)
                        ...
                        best_DF.to_csv(os.path.join('/media/zhemengwu/Gigantic Data/SingleContext_Hipp/FittedResult',
    ...
    def __fitSingleBatch(self, alpha1, alpha2, sigma, v):
        tmp_DF = pd.DataFrame(columns=["Alpha1", "Alpha2", "Sigma", "v", "dW", "dW_portion", "W_Port_R", "W_other", "SSE"])
        for dW in self.dW:
            for dW_portion in self.dW_portion:
                for W_port_R in self.W_port_R:
                    ...
                    for W_other in self.W_other:
                        ...
                        rat = Participant(alpha=[alpha1, alpha2], c=0, v=v, sigma=sigma,
                        ...
                        result_slice = {"Alpha1": alpha1, "Alpha2":alpha2,
                                        "Sigma": sigma, "v": v,
...
if __name__ == "__main__":
    data_path = "/media/zhemengwu/Gigantic Data/SingleContext_Hipp/Data/"
    result_path = data_path.replace("/Data/", "/FittedResult/")
    if not os.path.exists(result_path):
        os.mkdir(result_path)

    fit = Fitter(data_path)
    best_DF = fit.fit()
    best_DF.to_csv(os.path.join(result_path, "Rat{n}_Result.csv".format(n=fit.ratID)), index=False)
    ...
    best_rat = Participant(alpha = [best_DF["Alpha1"].iloc[0], best_DF["Alpha2"].iloc[0]],
                           sigma = best_DF["Sigma"].iloc[0],
                           v = best_DF["v"].iloc[0],
                           W_L = [W_port_L, W_SA_L, best_DF["W_other"].iloc[0]],
                           W_R = [best_DF["W_Port_R"].iloc[0], best_DF["W_other"].iloc[0], best_DF["W_other"].iloc[0]],
                           use_multi = fit.multi)
