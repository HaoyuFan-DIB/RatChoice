...
for group in groups:
    print(group)
    if not os.path.exists(os.path.join(Rootpath, "AUC_Single", group)):
        os.mkdir(os.path.join(Rootpath, "AUC_Single", group))
    ...
    for rat_id in rat_dir.keys():
        ...
        AUC = calcAUC(X_rat, 1 - Y)   # normalized Y
        ...
        plt.xlabel(group)
        #plt.ylim([0, 5])
        plt.savefig(os.path.join(Rootpath, "AUC_Single", group, rat_id + "-" + rat_dir[rat_id] + ".png"))
        #plt.show()
        plt.close()
    ...
    out_filename = os.path.join(Rootpath, "AUC_Single", "AUCResult_" + group + ".csv")
