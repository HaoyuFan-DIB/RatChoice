# Test if SqA data can be fit with a Weibull_CDF
# plus a reverse Weibull_CDF (X' = 28 - X)
...
for group in groups:
    ...
    pic_outdir = os.path.join(Rootpath, group+"_reverse")
    ...
    for rat_id in rat_dir.keys():
        ...
        model1 = Weibull_cdf(name="Weibull_CDF_Plus", base=np.e)
        model2 = Weibull_cdf(name="Weibull_CDF_Minus", base=np.e, reverse=True)
        SqA_Model = model1 + model2
        ...
        SqB_Model = Weibull_PDF(lambda_0 = 20)
        SqB_Model.A.min = -99999
        # SqB_Model.C.val = 0.0
        # SqB_Model.C.freeze()
        SqB_Model.delay.val = 0.0
        SqB_Model.delay.freeze()
        SqB_Model = SqB_Model - 1
        ...
        A_SSE = np.sum((Y_SqA - SqA_Model(X))**2)
        B_SSE = np.sum((Y_SqB - SqB_Model(X))**2)
        # Plot
        fig = plt.figure(figsize=[8, 6], dpi=200)
        ax = fig.add_subplot(1, 1, 1)
        ax.scatter(X, Y_SqA, color="r")
        ax.plot(X_plot, SqA_Model(X_plot), color="r", label="SqA, SSE=%.2f" % A_SSE)
        ax.scatter(X, Y_SqB, color="b")
        ax.plot(X_plot, SqB_Model(X_plot), color="b", label="SqB, SSE=%.2f" % B_SSE)
